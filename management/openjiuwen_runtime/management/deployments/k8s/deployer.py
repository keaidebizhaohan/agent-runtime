...
class K8sDeployer(Deployer[K8sParams]):
    ...
    async def deploy(self, ctx: DeployContext[K8sParams]) -> DeployResult:
        ...
        try:
            settings = self._resolve_runtime_settings(ctx)

            config_file_name, config_content = self._resolve_ir_file(ctx)

            labels = {
                "app": settings.deployment_name,
            ...
            env_vars = {
                ...
                "CONTAINER_PORT": str(settings.container_port),
            }
            ...
            await self._create_or_patch_secret(core_api, settings.namespace, secret_body)
            await self._create_or_patch_deployment(apps_api, settings.namespace, deployment_body)
            await self._create_or_patch_service(core_api, settings.namespace, service_body)

            self._deployments[deployment_id] = settings.deployment_name
            success, output = await self._wait_for_rollout(apps_api, settings.namespace,
                                                           settings.deployment_name, settings.replicas)
            if not success:
                return DeployResult(
                    success=False,
                    deployment_id=deployment_id,
            ...
            if not url:
                ...
                if settings.node_port is not None:
                    expose_port = int(settings.node_port)
                elif kube_port is not None:
                    expose_port = int(kube_port)
                else:
                    expose_port = settings.service_port

...
class _RecursionState:
    """``run_agent`` 递归续轮间传递的内部状态（参考 G.FNM.03）。

    ``step_counter`` 为跨递归共享的可变步骤计数器（单元素 list，原地自增）；
    ``reference_task_ids`` 为 cascade 续轮声明父子任务关联用的子任务 id 列表。
    将二者打包为具名对象，避免 ``run_agent`` 参数个数超限。
    """
...
class Executor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        await self.run_agent(
            ...
            original_body=original_body,
            cascade_result=None,
            # cascade 递归共享同一计数器（_RecursionState 默认 step_counter=[0]）
            recursion=_RecursionState(),
        )
    ...
    async def run_agent(
        ...
        recursion: Optional[_RecursionState] = None,
    ) -> None:
        if recursion is None:
            recursion = _RecursionState()
        step_counter = recursion.step_counter
        reference_task_ids = recursion.reference_task_ids
            ...
            if isinstance(event, DelegateRequest):
                ...
                if va_result is not None:
                    await self.run_agent(
                        ...
                        recursion=_RecursionState(step_counter=step_counter),
                    )
                elif finalized:
